...
export class PostsComponent implements OnInit {
  ...
  getPosts() {
    this.http
      .get(`http://13.231.177.120/api/getpost/${this.user.topic}`)
      .subscribe((res: any) => {
        this.contents = res;
      });
  }
  ...
  deletePost(post: Post) {
    this.http
      .delete(`http://13.231.177.120/api/deletepost/${post._id}`)
      .subscribe((res: any) => {
  ...
  getTopicChange(data: string) {
    ...
    if (data == 'explore') {
      ...
      this.showCreate = false;
      this.http
        .get(`http://13.231.177.120/api/getpostuser/${this.user._id}`)
        .subscribe((res: any) => {
          this.contents = res;
          console.log(res);
        });
  ...
  addStory() {
    this.http
      .post<any>('http://13.231.177.120/api/createstory', {
