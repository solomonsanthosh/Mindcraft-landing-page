...
export class MeetRequestsComponent implements OnInit {
  ...
  getUserRequest() {
    this.http
      .get(`http://13.231.177.120/api/getuserrequest/${this.user._id}`)
      .subscribe((res: any) => {
        console.log(res);
        res.map((element: any) => {
  ...
  sendPayment() {
    this.http
      .post('http://13.231.177.120/api/payment', {
        name: this.user.name,
        ...
        await this.stripe
          ...
          .then((result: any) => {
            if (result.error) {
              ...
              this.http
                .put(
                  `http://13.231.177.120/api/makepayment/${this.currentRequest._id}`,
                  {
  ...
  createReview() {
    ...
    this.http
      .post('http://13.231.177.120/api/review', {
